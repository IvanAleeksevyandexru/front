  ...
  ngOnInit(): void {
    if (this.data.length) {
      ...
      if (!this.isCompleteData) {
        const isValidDate = moment(this.child.rfBirthCertificateIssueDate, DATE_FORMAT).isValid();
        this.child.rfBirthCertificateIssueDate = isValidDate
          ? moment(this.child.rfBirthCertificateIssueDate, DATE_FORMAT).toDate()
          : moment().toDate();
      }
    }
  }
            ...
            if (change[key]) {
              switch (key) {
                case 'rfBirthCertificateIssueDate':
                  this.child[key] = moment(change[key], DATE_FORMAT).toDate();
                  break;
                default:
