...
export class CompressionService {
  ...
  */
  private getFilefromDataUrl(dataUrl: string, filename: string, lastModified: number = Date.now()) {
  ...
  * @returns {HTMLCanvasElement | OffscreenCanvas}
  */
  private handleMaxWidthOrHeight(canvas: HTMLCanvasElement | OffscreenCanvas, options): HTMLCanvasElement | OffscreenCanvas {
  ...
  */
  private getNewCanvasAndCtx(width: number, height: number) {
    let canvas;
    let ctx;
    try {
 ...
 */
  ...
  */
  private async compress(file: File, options: Options): Promise<File | Blob> {
    ...
    const orientationFixedCanvas = maxWidthOrHeightFixedCanvas;
    let quality = 1;

    let tempFile = await this.canvasToFile(orientationFixedCanvas, options.fileType || file.type, file.name, file.lastModified, quality);
    ...
    while (remainingTrials-- && (currentSize > maxSizeByte || currentSize > sourceSize)) {
      ...
      compressedFile = await this.canvasToFile(newCanvas, options.fileType || file.type, file.name, file.lastModified, quality);
